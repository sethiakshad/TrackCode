...
import { getFriendsLeaderboard } from '../../lib/api/friendApi';
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto text-left">
      ...
      {loading ? (
        ...
        <div className="h-64 flex items-center justify-center text-dark-textMuted text-sm">
          No friends found. Send some friend requests to start competing!
        </div>
      ) : (
      ...
      <Card className="border-white/5 bg-slate-900/40 backdrop-blur-xl">
        <CardHeader>
          ...
          <CardDescription>Standings of all connected peers</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
                ...
                <div key={u.rank} className="flex items-center justify-between p-3.5 bg-slate-950/40 border border-white/5 rounded-xl transition-all hover:border-white/10">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="text-left">
                      <p className="text-xs font-bold text-white">{u.name} {u.isCurrentUser && <span className="text-primary-400">(You)</span>}</p>
