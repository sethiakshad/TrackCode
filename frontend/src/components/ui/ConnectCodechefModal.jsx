...
import { fetchCodechefProfile } from '../../services/codechefService';
import { useAuth } from '../../context/AuthContext';

export const ConnectCodechefModal = ({ isOpen, onClose, onConfirm }) => {
  const { user } = useAuth();
  ...
  const handleConfirm = async () => {
    ...
    setVerificationError('');
    try {
      const freshData = await fetchCodechefProfile(username);
      const targetCode = verificationCode.toLowerCase();
      
      const containsCode = JSON.stringify(freshData).toLowerCase().includes(targetCode);

      if (!containsCode) {
        setVerificationError(`Verification code "${verificationCode}" not found. Paste it in your About Me or Country/City on CodeChef.`);
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-white/5 px-6 py-4">
              <div className="flex items-center space-x-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-amber-700 to-amber-900 shadow-lg shadow-amber-900/25">
                  <span className="font-bold text-white text-lg font-serif">CC</span>
                </div>
                <div>
                  <h3 className="text-sm font-bold text-white">Connect CodeChef</h3>
                  <p className="text-[11px] text-slate-400">Link your CodeChef profile</p>
                </div>
              </div>
              <button
                onClick={handleClose}
            ...
            <div className="px-6 py-5">
              {step === 'input' && (
                <form onSubmit={handleFetch} className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-400">CodeChef Username</label>
                    <Input
                      type="text"
                    ...
                    <p className="text-[11px] text-slate-500">
                      Enter your CodeChef username to fetch your profile data.
                    </p>
                  </div>
              ...
              {step === 'loading' && (
                <div className="flex flex-col items-center justify-center py-8 space-y-4">
                  <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-amber-700/20 to-amber-900/20 flex items-center justify-center">
              ...
              {step === 'preview' && profileData && (
                <div className="space-y-4 text-left">
                  ...
                  <div className="flex items-center space-x-3 rounded-xl border border-white/5 bg-slate-900/60 p-3">
                    <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-amber-700 to-amber-900 flex items-center justify-center text-white font-bold text-sm">
                  ...
                  <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3.5 space-y-2.5">
                    ...
                    <div className="text-[11px] text-slate-300 space-y-1.5 leading-normal">
                      <p>Copy the code below and paste it anywhere on your CodeChef profile (e.g. <b>About Me</b> or <b>Name</b>):</p>
                      <p>Save changes on CodeChef and click verify:</p>
                    </div>
